...
interface BuildResult {
  ...
  tag?: string;
  successfulBuildCommand?: string[];
  ranBuildFrom?: string;
  isBabel?: boolean;
  isReact?: boolean;
  isPackedFromBuildDir?: boolean;
  usesCleanPublish?: boolean | string;
...
try {
  ...
  // accordingly later.
  let packageSubdir: string | null = null; // null means this is NOT a multipackage monorepo
  ...
  if (packageName.startsWith("@types/")) {
    ...
    let tagExisted: string | boolean = false;
    for (const tagName of possibleTagNames) {
      try {
        ...
        if (e instanceof Error) {
          console.log("stdout:", (e as any).stdout);
          console.log("stderr:", (e as any).stderr);
        } else {
          console.log("Error:", e);
        }
      }
    ...
    buildResult.tag = tagExisted as string;
  ...
  let subdirPackageJson: PackageJson | undefined;

  if (packageSubdir) {
    subdirPackageJson = JSON.parse(
      await readFile(`${packageSubdir}/package.json`, "utf-8"),
    );
  }

  let hasBuilt = false;
  async function attemptBuild(cmd: string[], subdir?: string) {
