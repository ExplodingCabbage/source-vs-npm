...
async function auditPackage(packageName) {
  ...
  try {
    ...
    try {
      ...
      // 2. Some packages (e.g. cliui) publish tsbuildinfo, an intermediate
      //    build artifact from TypeScript compilation that doesn't reliably
      //    end up with the same content when rebuilding from scratch. We
      //    ignore any differences in that file:
      // TODO: Implement 2
      // TODO: Do this by rewriting the logic here to parse the diff and create
      //       a list of file paths with changes (& whether "change" = add, delete, or edit),
      //       then loop over that list and apply the rules. Doing parsing
      //       within the individual rules 1 2 and 3 is fiddly and hard to extend.
